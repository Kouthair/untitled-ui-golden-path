...
Your source of truth for "How a button looks" is NOT Figma. It is the Storybook stories and the vendor/ui components.
...
Client-specific brand overrides are in src/styles/theme-overrides.css.
RULE: You MUST use semantic Tailwind classes (e.g., bg-brand-solid, text-primary).
...
Use the strict type scale: text-xs, text-sm, text-md, text-lg, text-xl.
...
Look up the component's Storybook story to see its exact argTypes and variants.
